...
const FlatNORM = squareTexture({size:4,color:[0.5,0.5,1.0],name:'defnormal'}); 
...
const normalModel = new THREE.TextureNode(FlatNORM);
...
const tslParallaxFX = Fn(({ material, geometry, object })=>{
	if (!material.userData.hasOwnProperty("ParallaxTX")){
		return FXColor;
	}
...
const tslMultilayerColor = Fn( ( { material, geometry, object } ) => {
	const definitiveColor = material.userData.diffuseTexture.mul(material.userData.layerColor);
	If (material.userData.MLSBInspect,()=>{ definitiveColor.assign(vec3(1,0,0)) });
	return definitiveColor;
});

const tslMultilayerOpacity = Fn(({material}) => {
	return material.userData.alphaMap.r;
});
...
const tslGDNormals = Fn(( { material } ) => {
	const normalized = material.userData.globalNormalTexture
	return transformNormalToView(normalMap(normalized, float(material.userData.detailNormalScale)),"v_normalViewGeometry");
})
...
const tslMetalness = Fn(( { material, geometry, object } ) => {
	let MFactor = uniform(material.userData.metalnessScale)
	return material.userData.metalnessTexture.mul(MFactor);
})
	...
	switch (mapName){
		...
		default:
			if (mapName!="engine\\textures\\editor\\grey.xbm"){
				...
				if ((type=="normal") || (type=="globalnormal")) { return FlatNORM; }
			}
			return GRAY;
	}
}
...
function typeTexturePass(prop){
	if (prop?.isTextureNode){
		return prop.value;
...
function tslUpdateUVTransform(){
	try {
		...
		editUV(typeTexturePass(materialStack[selected].userData.diffuseTexture));
		...
		if (materialStack[selected].roughnessMap?.isTexture){
			editUV(materialStack[selected].roughnessMap)
		}
		...
		if (materialStack[selected].metalnessMap?.isTexture){
			editUV(materialStack[selected].metalnessMap)
		}
		/* if (materialStack[selected].userData.normalDetailMap?.isTexture){
			editUV(materialStack[selected].userData.normalDetailMap.value);
		} */
		editUV(typeTexturePass(materialStack[selected].userData.detailNormalMap))
...
function img_screen_name(filename=""){
	if (filename==""){
		return `screen_${ new Date().valueOf()}.png`;
	}
	return `screen_${String(filename).split("\/").reverse()[0].split(".")[0]}_${ new Date().valueOf()}.png`;
}
...
function animate() {
    ...
    if(TDengine.getScreenshot == true){
		...
		a.href = imgDataShot;
		a.download = img_screen_name($("#modelTarget").val());
	}
...
async function _getTextureContent(filename){
	return new Promise((resolve,reject)=>{
		if (PARAMS.textureDebug){ console.log(`request _getTextureContent`,textureObj); }
			...
			.then((textureResult)=>{
				...
				if (fileInfo.format=='DDS'){
					...
					TODO need to be fixed because it search for the TextureObj
					 ...
					 correct the gamma*/
					return pngResolve(encodePng(textureResult),fileInfo)
				}
		}).then((filecontent)=>{
			//TODO need to be reconstructed the file
			resolve({data:filecontent,info:fileInfo,name:filename});
		}).catch((error)=>{
			reject(false)
		...
		theTextureContent.then((textureResult)=>{
			...
			if (!textureObj.hasOwnProperty('info')){
				try {
					...
					dummy.filename = textureObj.file;
	...
	try {
		if (textureObj.hasOwnProperty("file") && textureObj.hasOwnProperty("shader") && textureObj.hasOwnProperty("maptype") && textureObj.hasOwnProperty("entries")){
			...
			textureObj.entries.forEach((toMap)=>{
				if (PARAMS.textureDebug){
					console.log(toMap.shader,textureObj.file,toMap.maptype)
				}
				TSLsetTextureKind(textureStack[textureMD5Code],toMap.maptype,toMap.shader)
			})
		}else{
...
function calcTHREEFormat(info){
	...
	console.log(format == THREE.RGFormat);
	return format;
}
...
function encodeDecals(materialEntry,_materialName){
...
function encodeMetalbase(materialEntry,_materialName){
	...
	if (materialEntry.Data.hasOwnProperty('Normal')){
		...
		let normalValue = parseFloat(materialEntry.Data.NormalStrength).toPrecision(2)
		rbaseConfig.normalScale =  new THREE.Vector2(normalValue,normalValue);
	}
	...
	Rbase.roughnessMap.needsUpdate = true;
	Rbase.metalnessMap.needsUpdate = true;
...
function encodeMultilayer(materialEntry,_materialName){
	...
	Mlayer.userData={
		...
		detailNormalMap : new THREE.TextureNode(FlatNORM),
		name:_materialName,
		type:'multilayer',
		GlobalNormal: null,
		globalNormalTexture: new THREE.TextureNode(FlatNORM),
		layerColor : uniform(color(new THREE.Color(0x808080))),
		UVtiles: uniform(1.0),
		diffuseTexture : new THREE.TextureNode(WHITE),
		roughnessTexture : new THREE.TextureNode(WHITE),
		roughnessScale : 1.0,
		metalnessTexture : new THREE.TextureNode(BLACK),
		metalnessScale : 1.0,
		opacityMask : uniform(1.0)
	};

	Mlayer.userData.MLSBInspect = uniform(false);
	Mlayer.colorNode = tslMultilayerColor();
	Mlayer.roughnessMap = WHITE;
	Mlayer.roughness = 1.0;
	Mlayer.metalnessMap = BLACK;
	Mlayer.metalness = 1.0;
	...
	if (materialEntry?.Data.hasOwnProperty('GlobalNormal')){
		...
		if (textureStack[normMD5Code]===undefined){
			...
			console.log("GlobalNormal already there",textureStack[normMD5Code]);
		}
	}else{
	...
	if (materialEntry?.Data.hasOwnProperty('GlobalNormalIntensity')){
		Mlayer.userData.GlobalNormal = vec2(parseFloat(materialEntry?.Data.GlobalNormalIntensity).toPrecision(3));
	}

	/* if (Mlayer.hasOwnProperty("normalMap")){
		Mlayer.normalMap.needsUpdate = true;
	} */
	Mlayer.normalNode = normalMap(Mlayer.userData.globalNormalTexture,1.0) //tslGDNormals();
...
function encodeParallax(shader,caller){
	...
	if (shader.Data.hasOwnProperty("ScanlineTexture")){
		var scanlineID = getEncodedFileName(shader.Data.ScanlineTexture);
		if (textureStack[scanlineID]===undefined){
			textureStack[scanlineID] = retDefTexture(shader.Data.ScanlineTexture,shader.Name,"scanline");
			textureStack[scanlineID].wrapS = THREE.RepeatWrapping
			textureStack[scanlineID].wrapT = THREE.RepeatWrapping
			textureStack[scanlineID].userData.type="scanline";
			caller.userData.ScanlineTX = textureStack[scanlineID]
		}else{
			caller.userData.ScanlineTX = textureStack[scanlineID]
		}
	}
	if (shader.Data.hasOwnProperty("ScrollMaskTexture")){
		var scrollMaskID = getEncodedFileName(shader.Data.ScrollMaskTexture);
		if (textureStack[scrollMaskID]===undefined){
			textureStack[scrollMaskID] = retDefTexture(shader.Data.ScrollMaskTexture,shader.Name,"scrollmask");
			textureStack[scrollMaskID].wrapS = THREE.RepeatWrapping
			textureStack[scrollMaskID].wrapT = THREE.RepeatWrapping
			textureStack[scrollMaskID].userData.type="scrollmask";
			caller.userData.ScrollMaskTX = textureStack[scrollMaskID]
		}else{
			caller.userData.ScrollMaskTX = textureStack[scrollMaskID]
		}
	}

	if (shader.Data?.ScrollSpeed1 > 0) {

		if (shader.Data.hasOwnProperty("ScrollVerticalOrHorizontal")){
			console.log(shader.Data.ScrollVerticalOrHorizontal==1 ? "horizontal" : "vertical")
		}
	}
...
function encodeFX(materialEntry,_materialName){
	...
	if (materialEntry.MaterialTemplate=="base\\fx\\shaders\\parallaxscreen.mt"){
		...
		// for scanlines https://github.com/mrdoob/three.js/blob/master/examples/webgpu_postprocessing_retro.html
...
function encodeGlass(materialEntry,_materialName){
	var glassMe = materialGlass.clone();
	glassMe.setValues({name:_materialName});
	return glassMe
}

function encodeHair(materialEntry,_materialName){
...
function encodeSkin(materialEntry,_materialName){
...
function codeMaterials(materialEntry,_materialName){
	if (materialEntry.hasOwnProperty('MaterialTemplate')){
		var shaderToMapName = MLSB.getShaderType(materialEntry.MaterialTemplate);
		switch (shaderToMapName){
			case "multilayer":
				return encodeMultilayer(materialEntry,_materialName);
				break;
			case "metal_base":
				return encodeMetalbase(materialEntry,_materialName);
				break;
			case "decals":
				return encodeDecals(materialEntry,_materialName);
				break;
			case "fx":
				return encodeFX(materialEntry,_materialName);
				break;
			case "glass":
				return encodeGlass(materialEntry,_materialName);
				break;
			case "hair":
				return encodeHair(materialEntry,_materialName);
				break;
			case "skin":
				return encodeSkin(materialEntry,_materialName);
				break;	
			case "unknown":
				return materialNone;
				break;
...
function purgeTextureList(textureList){
	try {		
		textureList.forEach((texture,index)=>{
			if ((textureStack.filter((tex)=>tex.userdata.file==texture)).length > 0){
				textureList.splice(index,1);
			}
		});
	} catch (error) {
		console.log(error);
		notifyMe(`Error during the purge`);
	}
	return textureList;
}
...
/**
 *
 * @param {array} textureList list of textures files to be processed
 * @param {boolean} forceLoad replace/reload textures
 * @returns array of results
 */
//TODO Insert it in the workflow for texture loading
function systemLoadTextures(textureList, forceLoad=false){
	var resultant = textureList.map((x)=> false);
	/**
...
function LoadMaterial(DBmaterial,checkTextures=true){
	return new Promise((resolve,reject)=>{
		...
		if (checkTextures){
			...
			textureSplicer(textureList,textureDock) /* TODO to be checked */
		}
...
function getAppearanceMaterials(appearances,index=0){
	return appearances[index].Materials
}
		...
		try {
			if (tempMaterial!=''){
				...
				if (materialLoaded = materialJSON.import(tempMaterial)){
					...
					/* //retrieve for the first apmaterialJSON.Appearancespearance the materials
...
function TSLsetTextureNode(texture,kind,selected){
	switch (kind){
		case 'alpha':
		case 'mlmask':
			materialStack[selected].alphaMap = texture;
			materialStack[selected].alphaMap.colorSpace = NoColorSpace;
			break;
		case 'diffuse':
		...
		case 'roughness':
			...
			break;
		case 'metal':
			materialStack[selected].metalnessMap = texture;
			break;
		case 'normal':
		case 'globalnormal':
			materialStack[selected].normalMap = texture;
			break;
		case 'detailnormal':
			materialStack[selected].userData.detailNormalMap.value = texture;
			break;
	}
}

function legacySetTexture(texture,kind,selected){
	switch (kind){
		case 'alpha':
		case 'mlmask':
			materialStack[selected].alphaMap = texture;
			...
			break;
		case 'diffuse':
			materialStack[selected].map = texture;
			break;
		case 'emissive':
			materialStack[selected].emissiveMap = texture;
			break;
		case 'roughness':
			materialStack[selected].roughnessMap = texture;
			break;
...
function TSLsetTextureKind(texture,kind,selected){
	texture.needsUpdate = true;
	if (texture?.isTextureNode){
		TSLsetTextureNode(texture,kind,selected)
	}else{
		legacySetTexture(texture,kind,selected)
	}
}
...
function TSLresetTextureKind(kind,selected){
	switch (kind){
		case 'diffuse':
			materialStack[selected].userData.diffuseTexture.value = GRAY;
			materialStack[selected].userData.diffuseTexture.update();
			break;
		case 'roughness':
		...
		case 'metal':
			...
			break;
		case 'normal':
			materialStack[selected].userData.detailNormalMap.value = FlatNORM;
			materialStack[selected].userData.detailNormalMap.update();
			break;
	}
}
...
$("#thacanvas").on("mouseover",function(event){
	...
	*/
				...
				textureKinds.forEach((kind,id)=>{
					if (materialTextureFile = materialCheckAttribute(kind,layerMaterial)){
						...
						if (searchMap<0){
							...
							console.log(`Set ${kind} at ${searchMap} level`);
							if (isBLACK(materialTextureFile)){ TSLsetTextureKind(BLACK,kind,selected); }
							if (isWHITE(materialTextureFile)){ TSLsetTextureKind(WHITE,kind,selected); }
							if (isGREY(materialTextureFile)){ TSLsetTextureKind(GRAY,kind,selected); }
						}
					}else{
						if (PARAMS.textureDebug){
	...
	if (sceneLoaded()){
		...
		if (source == 'ui'){
			tileValue = Number($("#layerTile").val());
		}
		var offsetX = parseFloat(source =='ui' ? $("#layerOffU").val() : mLsetup.Layers[MLSB.Editor.layerSelected].offsetU).toPrecision(4);
		var offsetY = parseFloat(source =='ui' ? $("#layerOffV").val() : mLsetup.Layers[MLSB.Editor.layerSelected].offsetV).toPrecision(4);
		var offset = calcOffset(tileValue,offsetX,offsetY) //new THREE.Vector2(h,(v==-0 ? 0 : v));
				...
				var test = _getFileContent({
					...
					.then((result)=>{
						...
						if (myMask?.length==1){
							...
							//materialStack[selected].userData.opacityMask.value = textureStack[nameMask];
	...
	if (selected!=undefined){
		materialStack[selected].userData.MLSBInspect.value = highlight;
		materialStack[selected].needsUpdate=true;
	}
